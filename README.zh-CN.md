...
1. **补录答疑主舞台演示** — 上传代码/截图 → AI 讲义 → 左右分栏（当前学生端 demo 还没这段）
2. **Cursor 实操更稳** — 等 Agent 完整跑通再停；减少中途截断；各框架完成标志更清楚
3. **学生端一键演示** — 用 `chapter` / `t` / `play` 参数脚本化录 watch，方便更新 README 演示
4. **安装体验** — 补齐 `requirements.txt` / 锁文件，以及「第一次跑通」检查清单

中期（能力与质量）：

5. **知识检索** — 可选向量库（如 Chroma）覆盖课纲笔记 + 学员上传，答疑更贴上下文
6. **多模型引擎** — DeepSeek 之外可插拔；导播台引擎状态更清晰
7. **跨平台录屏** — 降低对 macOS 窗口/整屏录制的硬依赖
8. **章节导出包** — 单章 timeline + 素材可打包带走，不必整棵 `recordings/`

更后（看需求再做）：

9. **商用授权路径** — Issue / 邮件申请付费部署、私有化、白标
10. **公开试看** — 可选只读学生端托管样例课（无密钥、无学员隐私）

欢迎通过 [Issues](https://github.com/wincap/recording-kg/issues) 提反馈或 PR。
